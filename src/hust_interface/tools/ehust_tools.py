...
def register_ehust_tools(mcp):
    ...
    @mcp.tool(
        ...
        description="Tra cứu Đề cương chi tiết học phần, Mô tả tóm tắt, Nội dung tóm tắt môn học, Sách giáo trình, Sách tham khảo, Slide bài giảng và file đề cương đính kèm của một mã học phần (ví dụ: 'MI1111', 'IT3040') hoặc mã lớp (ví dụ: '172154')."
    )
    async def get_course_syllabus(course_id: str) -> Dict[str, Any]:
        syllabus = await crawler.get_course_syllabus(course_id_or_uid=course_id)
